...
class ManageConnector(object):
    ...
    def registe_connector(self, server_switch):
        """
        @describe 注册连接器
        ...
        registers = dict()
        for server_alias in server_switch: registers[server_alias] = getattr(connector_parsers[server_alias.split("_")[0]], "Service")
        if registers: self.server_register = SimpleNamespace(**registers)


    def registe_handler(self, server_switch):
        """
        @describe 注册处理器
            @param server_switch:  链接服务开关
        """
        ...
        registers = dict()
        for server_alias in server_switch: registers[server_alias] = getattr(handler_parsers[server_alias.split("_")[0]], "MqSect")
        if registers: self.handler_register = SimpleNamespace(**registers)
    ...
    def conn_server(self, server_alias, server_options):
        ...
        """
        if not server_alias:
            raise ExceptionWarning("Please add a linked data source ??????? ?_? ")
